...
- **Magnifier** — drag to frame the area to enlarge and an enlarged view is placed next to it: rectangle / rounded / circle, with adjustable zoom (1.2–10×), border thickness and colour, connector, shadow, smoothing (off gives the hard pixel grid), and whether the enlarged content includes other annotations; both the view **and the framed source can be dragged at any time** — moving the frame changes what is enlarged and the connector re-aims itself; the wheel re-zooms the selected magnifier
